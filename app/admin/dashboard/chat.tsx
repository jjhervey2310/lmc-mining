  ...
  return (
    <div className="flex h-[420px] flex-col">
      <div ref={scroller} className="flex-1 space-y-2 overflow-y-auto pr-1">
        ...
        {messages.map((m, i) => (
          <div key={i} className={`whitespace-pre-wrap text-[12px] leading-relaxed ${m.role === 'user' ? 'text-amber-400' : 'text-neutral-300'}`}>
            <span className="mr-1 text-[10px] uppercase tracking-widest text-neutral-600">{m.role === 'user' ? 'you' : 'pa'}</span>
            {m.content}
          </div>
        ))}
        {busy && <div className="text-[12px] text-neutral-600 animate-pulse">pa is checking…</div>}
      </div>
      <div className="mt-2 flex gap-1 border-t border-neutral-800 pt-2">
        <input
          ...
          className="min-w-0 flex-1 border border-neutral-800 bg-black px-2 py-1.5 font-mono text-[12px] text-neutral-200 outline-none focus:border-amber-500"
        />
        <button onClick={send} disabled={busy} className="border border-amber-500 px-3 py-1.5 text-[11px] uppercase text-amber-500 hover:bg-amber-500 hover:text-black disabled:opacity-40">
          Send
        </button>
      </div>
    </div>
