...
- Scheduling: Supabase pg_cron + pg_net (NOT Vercel crons). 6 jobs as of 2026-09-21 cleanup: ACTIVE — daily-hashprice-snapshot (0 0), weekly-newsletter (0 15 Sun), weekly-analytics-review (Mon 02:00), comp-snapshot (20:05, feeds the AI Competition equity curve); PAUSED (active=false, kept for revival) — runner-scout (hourly), desk-digest-8am (14:00). All UTC. DELETED 2026-09-21 (recreate via cron.schedule + x-content-secret from Vercel env if the content machine revives): daily-content-drop, make-content-warm, heygen-quota-alert, morning-brief, job-verify, comp-trader, desk-watcher-15m. CAUTION: pg_net is fire-and-forget, so cron.job_run_details says "succeeded" even when the endpoint fails — check the target table for the row, never trust the run log alone
